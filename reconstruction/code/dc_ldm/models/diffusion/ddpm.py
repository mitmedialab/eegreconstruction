...
class DDPM(pl.LightningModule):
    ...
    def full_validation(self, batch, state=None):
        ...
        #all_samples = (50,6,3,256,256) where 50 = n_samples in validation
        ...
        #BE CAREFUL: the model is saved depending on whether the metric is HIGHER than the best_val
    ...
    def get_eval_metric(self, samples, avg=True):
        """
        samples: (50,6,3,256,256) where 50 = n_samples in validation
        """
        ...
        samples_to_run = np.arange(1, len(samples[0])) if avg else [1] #array([1,2,3,4,5])
        for m in metric_list:
            res_part = []
            for s in samples_to_run:
                pred_images = [img[s] for img in samples] #img[s] is the s-th copy of the n-th sample
                pred_images = rearrange(np.stack(pred_images), 'n c h w -> n h w c') #out: 50x256x256x3
                res = get_similarity_metric(pred_images, gt_images, method='pair-wise', metric_name=m)
                res_part.append(np.mean(res))
            res_list.append(np.mean(res_part))     
        res_part = []
        
        #evaluation "class" (changed to metrics-only with pcc)
        for s in samples_to_run:
...
class LatentDiffusion(DDPM):
    ...
    @rank_zero_only
    @torch.no_grad()
    def on_train_batch_start(self, batch, batch_idx): #removed dataloader_idx to work with later pl versions
