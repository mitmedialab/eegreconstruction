...
# Author: PerceiveLab
# URL: https://github.com/perceivelab/eeg_visual_classification/blob/main/layers.py
...
class ConvLayer2D(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel, stride, padding, dilation, dropout):
        super().__init__()
        # TRY MOVE RELU AFTER CONV
...
class TemporalBlock(nn.Module):
    def __init__(self, in_channels, out_channels, n_layers, kernel_size, stride, dilation_list, in_size, dropout):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            ConvLayer2D(
                in_channels, out_channels, kernel_size, stride, padding[i], dilation_list[i], dropout
...
class SpatialBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_spatial_layers, stride, input_height, dropout):
        super().__init__()
       
       #NOTE: we edit the kernel and padding length to better fit with 8 channels
        # kernel_list = []
        ...
        stride = (1,1)
        padding = [(4,0), (3,0), (2,0), (1,0)]
        kernel_list = [(8,1), (6,1), (4,1), (2,1)]

        self.layers = nn.ModuleList([
            ConvLayer2D(
                in_channels, out_channels, kernel_list[i], stride, padding[i], 1, dropout
            ) for i in range(num_spatial_layers)
        ])
